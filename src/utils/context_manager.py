...
def batch_put_contexts(items: List[Dict[str, Any]]) -> None:
    ...
    """
    try:
        logger.log_debug("DynamoDB 배치 저장 시도", {
        ...
        logger.log_debug("DynamoDB 배치 저장 성공", {"items_count": len(items)})

    except ClientError as e:
        error_code = e.response['Error']['Code'] if 'Error' in e.response else 'Unknown'
        error_msg = e.response['Error']['Message'] if 'Error' in e.response else str(e)

        logger.log_error(f"DynamoDB 배치 저장 중 오류 발생: {error_code}", e, {
            "items_count": len(items),
            "table_name": settings.DYNAMODB_TABLE_NAME,
            "error": error_msg
        })
...
def clear_cache() -> None:
    ...
    logger.log_debug("컨텍스트 캐시 비우기", {"cache_size": len(_context_cache)})
    _context_cache = {}
