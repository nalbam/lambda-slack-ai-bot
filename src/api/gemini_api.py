...
class GeminiAPI:
    ...
    def generate_image(
        ...
        try:
            ...
            if (
                "not enabled" in str(e)
                or "not supported" in str(e)
                or "allowlist" in str(e)
                or "403" in str(e)
            ):
                raise GeminiApiError(
    ...
    def generate_video(
        ...
        try:
            ...
            # 응답 로깅
            logger.log_info(
                "Gemini 비디오 생성 작업 시작됨",
                {
                    "operation_name": operation_name,
                    "operation_id": operation_id,
                    "operation_done": operation_done,
                    "duration": duration_seconds,
                    "response_type": type(operation).__name__,
                    "operation_attributes": [
                        attr for attr in dir(operation) if not attr.startswith("_")
                    ],
                },
            )
    ...
    def generate_speech(
        ...
        try:
            ...
            # 응답 로깅
            candidates = response.candidates if hasattr(response, "candidates") else []
            audio_data = candidates[0] if candidates else None
            ...
            logger.log_info(
                "Gemini TTS 완료",
                {
                    ...
                    "candidates_count": len(candidates),
                    "audio_data_type": (
                        type(audio_data).__name__ if audio_data else None
                    ),
                    "response_type": type(response).__name__,
                    "usage": getattr(response, "usage_metadata", None),
                },
    ...
    def analyze_image(
        ...
        try:
            ...
            logger.log_info(
                "Gemini 이미지 분석 요청", {"model": model, "prompt": prompt[:100]}
            )
...
def analyze_image_with_gemini(
    image_data: str, prompt: str, mime_type: str = "image/png"
) -> Dict[str, Any]:
